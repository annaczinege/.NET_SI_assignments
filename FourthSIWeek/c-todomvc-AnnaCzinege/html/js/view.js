class View {
    ...
    removeItem(id) {
        ...
        if (elem) {
            ...
            this.setItemsLeft(this.getItemsLeft() - 1);
        }
    }
    ...
    getItemsLeft() {
        return parseInt(this.$todoItemCounter.innerHTML.split(' ')[0]);
    }
    ...
    bindEditItemSave(handler) {
        View.delegate(this.$todoList, 'li .edit', 'blur', ({target}) => {
            if (!target.dataset.iscanceled) {
                handler(View.itemId(target), target.value.trim(), target.parentElement.classList.contains("completed"));
            }
        }, true);
